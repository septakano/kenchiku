...
@Service
public class MstOwnerManagementService implements MstSearchService<MstOwnerManagementForm, MstOwnerManagementForm> {
  ...
  public MstOwnerManagementService(MstOwnerManagementRepository mstOwnerRepository, MstCodeService mstCodeService,
      MstCustomerService mstCustomerService, ExcelFileService<MstOwnerManagement> excelFileService,
  ...
  public MstOwnerManagementForm findByIdConvertOwnerForm(Integer id) {
    MstOwnerManagement mstOwner = mstOwnerRepository.findByIdWithClientName(id).orElse(new MstOwnerManagement()); // ClientNameを含めたmstOwnerを1行取得
    MstOwnerManagementForm tmp = new MstOwnerManagementForm();
  ...
  public List<MstOwnerManagementForm> search(MstOwnerManagementForm mstOwnerForm) {
    ...
    List<MstOwnerManagement> a = mstOwnerRepository.search(individual, client, corporation, corporationKana,
        department, ownerName, ownerNameKana, postCode, prefectures, address, buildingName, phone,
        mobilePhone, email, status, createdAt, updatedAt);
    // 表示がフォームの人のみ下を記入
    List<MstOwnerManagementForm> mstOwnerFormList = new ArrayList<>();
  ...
  public void upload() {
    try {
      ...
      for (int i = 1; i <= num; i++) {
        mstOwnerRepository
            .save(this.excelFileService.importDataToRowTypeExcel(new MstOwnerManagement(), "owner", i));
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
